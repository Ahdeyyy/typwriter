// Formatting commands, ported from the desktop app's
// src/lib/typst-codemirror-lang/commands.ts so the mobile toolbar behaves the
// same way: marks toggle (wrap ⇄ unwrap) instead of blindly inserting, line
// prefixes toggle across the whole selection, and Enter continues list items.
// Only the commands the mobile toolbar actually uses are kept.
...
export const toggleMath = wrapWith("$");
...
/** Single-button heading toggle: heading 1 ⇄ normal text. */
export function toggleHeading(view: EditorView): boolean {
  const { state } = view;
  const line = state.doc.lineAt(state.selection.main.head);
  const isHeading = /^\s*=+\s/.test(line.text);
  return setHeadingLevel(isHeading ? 0 : 1)(view);
}
...
function toggleLinePrefix(detect: RegExp, marker: string): Command {
  return (view) => {
    ...
    const allMarked = relevant.length > 0 && relevant.every((l) => detect.test(l.text));
