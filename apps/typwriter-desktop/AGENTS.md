...
- `workspace/` — `WorkspaceState`, filesystem `watcher`, path helpers, recent-workspaces store. `watcher.rs` turns raw `notify` batches into typed `FileChange`s (created / modified / removed / renamed): it pairs the two halves of a rename by notify's tracker id, collapses the several events one write produces into one change, and settles each kind against the filesystem once the batch has gone quiet. It also keeps the compiler's main-file pointer alive through an external rename or delete. Every in-app mutation claims its paths with `note_self_write` *before* touching disk (`self_writes.rs`), so the app never reacts to its own writes.
