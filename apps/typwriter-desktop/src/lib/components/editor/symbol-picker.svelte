          ...
          {#each grouped as group (group.category)}
            ...
            <div class="grid grid-cols-8 gap-1">
              {#each group.symbols as symbol (symbol.name)}
                <button
                  type="button"
                  class="hover:bg-accent hover:text-accent-foreground flex aspect-square
                         flex-col items-center justify-center rounded-md text-lg"
                  title="{symbol.name} — inserts {insertionFor(symbol, inMath)}"
          ...
          <div class="grid grid-cols-8 gap-1 pt-1">
            {#each matches as symbol, index (symbol.name)}
              <button
                ...
                class="hover:bg-accent hover:text-accent-foreground flex aspect-square
                       flex-col items-center justify-center rounded-md text-lg
                       {index === 0 ? 'ring-accent-foreground/30 ring-1' : ''}"
                title="{symbol.name} — inserts {insertionFor(symbol, inMath)}"
                onclick={() => insert(symbol)}
              >
                {symbol.char}
