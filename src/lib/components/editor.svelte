        ...
        if (appContext.workspace && appContext.workspace.document) {
            if (documentExtension.ext === "typ") {
                ...
                if (view) {
                    ...
                    await appContext.workspace.document.getPreviewPosition(
                        cursor,
                    );
                }
                ...
                if (page < 1) {
                    appContext.workspace.renderedContent = (
                        ...
                        let img = new Image(page.width, page.height);
                ...
                if (render_response) {
                    let img = new Image(
                        render_response.width,
                        render_response.height,
                    );
                    img.src = `data:image/png;base64,${render_response.image}`;
                    appContext.workspace.renderedContent.splice(
                        page - 1,
                        1,
                        img,
                    );
